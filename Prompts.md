...
Claude Code、Codex现在工作的时候都支持多开子智能体。 用户基本上并不关心子智能体的工作状态； 而是仅关心当前的主任务（一般是主agent）是否已经结束。目前的策略，如果子智能体的工作结束，也会有提示，这会让人感觉很奇怪，也失去了提醒用户主任务已经结束的意义。你赶紧修复。  使用 awesome-code skill 辅助规划、优化。所有问题都要解决。 如果工作时有疑问，或者有更好的方案，自己选个最优方案优化，不要问我。不要破坏其它功能。要保证最终成品能正常、稳定、高效地工作。 注意：我要的是“某个回复结束就通知”，不是 SessionEnd。
