...
目前， 用户消息一发给Codex后，Codex接到通知立刻就会产生通知，而不是仅完成一轮回复时产生通知； 这样，一轮回复就有2次通知。 我觉得，完成一轮回复时产生通知就行，第1个通知不需要。 使用 awesome-code skill 辅助规划、优化。所有问题都要解决。 如果工作时有疑问，或者有更好的方案，自己选个最优方案优化，不要问我。不要破坏其它功能。要保证最终成品能正常、稳定、高效地工作。 
